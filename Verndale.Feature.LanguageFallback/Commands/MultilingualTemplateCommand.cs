...
namespace Verndale.Feature.LanguageFallback.Commands
{
    ...
    public abstract class MultilingualTemplateCommand : Command
    {
        ...
        protected virtual bool SetCheckboxFieldValue(Item item, ID fieldId, bool newCheckedValue = true)
        {
            ...
            if (item == null)
            {
                Log.Warn("LanguageFallbackTools.Fallback.EnableLanguageFallbackOnTemplateFields.SetFieldValue(): item is null, skipping...", this);
            ...
            if (field == null)
            {
                ...
                Log.Warn(string.Format("LanguageFallbackTools.Fallback.EnableLanguageFallbackOnTemplateFields.SetFieldValue(): Item {0} field '{1}' is null. skipping...",
                    item.Paths.FullPath,
                    fieldId),
                this);
            ...
            // Log the changes.
            Log.Info(
                string.Format("LanguageFallbackTools.Fallback.EnableLanguageFallbackOnTemplateFields.SetFieldValue(): Item {0}: Old value: '{1}'; New value: '{2}'; valueChanged: {3}",
                    item.Paths.FullPath,
                    field.Checked,
                    newCheckedValue,
                    valueChanged),
            ...
            if (valueChanged)
            {
                ...
                using (new Sitecore.SecurityModel.SecurityDisabler())
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        Log.Error(string.Format("LanguageFallbackTools.Fallback.EnableLanguageFallbackOnTemplateFields.SetFieldValue(): Item {0} ERROR: '{1}'",
                                item.Paths.FullPath,
                                ex.Message),
                            ex,
                        this);
        ...
        private string BuildCompletionMessage(int changeCount)
        {
            if (changeCount == 1)
            {
                return "Processing completed. 1 item was updated.";
            }
