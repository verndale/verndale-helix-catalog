...
namespace Verndale.Feature.LanguageFallback.UI
{
	public partial class LanguageFallbackReport : System.Web.UI.Page
	{
		...
		private string GetLanguages(Item item, Item filteredLanguageVersion, out Language forFilteredLanguageFallingBackTo, out bool forFilteredAllSpecifiedFieldsAreFallingBack)
		{
			var languageStr = new StringBuilder();
			forFilteredLanguageFallingBackTo = null;
			forFilteredAllSpecifiedFieldsAreFallingBack = false;
			...
			foreach (var languageVersion in versionList)
			{
				...
				if (languageVersion.Language != null && !String.IsNullOrEmpty(languageVersion.Language.Name))
				{
					...
					if (filteredLanguageVersion != null && filteredLanguageVersion.Language == languageVersion.Language)
					{
						forFilteredLanguageFallingBackTo = languageFallingBackTo;
						forFilteredAllSpecifiedFieldsAreFallingBack = allSpecifiedFieldsAreFallingBack;
					}
